...
zaraxt_db = '/Users/student/Downloads/zardaxt/zardaxt_db_formatted.json'
...
def load_zardaxt_db():
    ...
    if not databaseLoaded:
        ...
        with open(zaraxt_db) as f:
            dbList = json.load(f)
            for line in dbList:
                if line['os'] not in os_sample_count:
                    os_sample_count[line['os']] = 0
                os_sample_count[line['os']] += 1
...
# fields missing in all samples, omit instead of normalizing to 0
"""
...
def score_fp(fp):
    ...
    for entry in dbList:
        ...
        """
        list=[]
        if str(4*entry['tcp_off']) == fp['tcp_fingerprint']['tcp_header_length']:
            list.append("off")
            score += 2.5
        if str(entry['ip_ttl']) == fp['tcp_fingerprint']['ip_ttl']:
            list.append("ttl")
            score += 2
        if str(entry['tcp_window_size']) == fp['tcp_fingerprint']['tcp_window_size']:
            list.append("wsize")
            score += 2
        if str(entry['tcp_flags']) == fp['tcp_fingerprint']['tcp_flags']:
            list.append("flags")
            score += 0.25
        if str(entry['tcp_mss']) == fp['tcp_fingerprint']['tcp_mss']:
            list.append("mss")
            score += 1.5
        if str(entry['tcp_options']) == fp['tcp_fingerprint']['tcp_options']:
            list.append("options")
            score += 4
        if str(entry['tcp_options_ordered']) == fp['tcp_options_ordered']:
            list.append("order")
            score += 2.5
        print(fp['tcp_fingerprint']['tcp_window_scaling'])
        if str(entry['tcp_window_scaling']) == fp['tcp_fingerprint']['tcp_window_scaling']:
            list.append("wscale")
            score += 2
        print(score)
        print(list)
        ...
        for os_name in os_scores:
            avg_os_score[os_name] = round(
                os_scores[os_name] / os_sample_count[os_name], 5)
...
# modified to save result in JSONL
def make_os_guess(input_file, output_file):
    number = 1
    """
...
# wrote myself, file in JSONL
def run():
    input_file_path = '/Users/student/Downloads/hs_test.json'
    output_file_path = '/Users/student/Downloads/zardaxt/zardaxt_result.json'
    make_os_guess(input_file_path, output_file_path)
